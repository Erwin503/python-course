...
class Car:
    def __init__(self, model, vin, numbers):
        ...
        self.__numbers = numbers  # private attribute
        
        # Проверяем VIN номер и номер автомобиля при инициализации
        if not self.__is_valid_vin(vin):
            raise IncorrectVinNumber('Некорректный тип vin номер')
        if not self.__is_valid_numbers(numbers):
            raise IncorrectCarNumbers('Некорректный тип данных для номеров')

    # Метод для проверки корректности VIN номера
    def __is_valid_vin(self, vin_number):
        if not isinstance(vin_number, int):  # Проверка на целое число
            raise IncorrectVinNumber('Некорректный тип vin номер')
        if not (1000000 <= vin_number <= 9999999):  # Проверка на диапазон
            raise IncorrectVinNumber('Неверный диапазон для vin номера')
        return True

    # Метод для проверки корректности номера автомобиля
